...
@section('content')
<form class="mt-6" method="POST" action="{{route('books.update', $book->id)}}">
    @csrf
    @method('PUT')
    <div class="mb-6">
    ...
    <div class="mb-6">
        ...
        <input type="number" id="isbn" name="isbn"
            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
            required placeholder="ISBN Buku" value="{{$book->isbn}}" required>
    </div>
    <div class="mb-6">
    ...
    <div class=" mb-6">
        ...
        <input type="text" id="author" name="author"
            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
            required placeholder="Penulis" value="{{$book->author}}" required>
    </div>
    <div class=" mb-6">
        <label for="publisher" class="block mb-2 text-sm font-medium text-gray-900 ">Penerbit</label>
        <input type="text" id="publisher" name="publisher"
            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
            required placeholder="Penerbit" value="{{$book->publisher}}" required>
    </div>
    <div class=" mb-6">
        <label for="synopsis" class="block mb-2 text-sm font-medium text-gray-900 ">Sinopsis</label>
        <textarea id="synopsis" name="synopsis"
            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-5 h-20"
            required placeholder="Sinopsis Buku" required>{{$book->synopsis}}</textarea>
    </div>
